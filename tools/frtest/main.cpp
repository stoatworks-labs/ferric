...
//---------------------------------------------------------------------------
// Parameter automation and the render pipe.
//
// FFGL plugins in this fleet have their project video RENDERED rather than
// filmed, and the reason is structural: an FFGL plugin has no window and no UI
// of its own. Its control surface is Resolume's inspector, and driving Arena
// means clicking a clip grid and an effects browser that are both custom-drawn
// with nothing in the accessibility tree to address.
//
// So `--pipe` puts real frames through the real shipped plugin class and hands
// them back. What ends up on screen is genuinely this plugin's output, from the
// same objects the bundle is built from -- but it is a render and not a screen
// recording, and the video's end card says so.
//
// A cue sheet is a plain text file of `frame  Parameter Name  value` lines.
// Values are held before the first key and after the last, and interpolated
// linearly between, so the piece is edited by editing the cue sheet rather than
// by editing code.
//
// ⚠️ **Option and boolean parameters must STEP.** An option is read by rounding
// and a boolean by a threshold, so a ramp between two settings passes through
// every setting in between -- a slow move from Cassette to Failing renders Reel
// to Reel and Video Head on the way. Two keys one frame apart put the change on
// the frame the cut chose.
//---------------------------------------------------------------------------
...
/**
    The drive the plugin would otherwise be given, generated.

    This plugin reacts to sound and the finished cuts are silent by design -- the
    pipeline writes a voiceover script rather than a soundtrack. So the rhythm is
    synthesised and written into the FFT parameter the way a host writes it, with
    the transport driven to match: a kick on every beat, hats on the eighths and
    a held note under the bar at 120 bpm.

    That makes the reaction on screen real -- the same `drive::compute` on the
    same kind of numbers, and the lurch lands on the grid because the grid is
    real too. What it is not is a recording of the plugin listening to music.

    Decays rather than gates: a square pulse makes the reaction look like a
    parameter being switched rather than something following a sound.
*/
void injectRhythm( Ferric& plugin, double seconds )
{
	...
	for( int i = 0; i < drive::kAudioBins; ++i )
		plugin.SetParamElementValue( Ferric::PT_AUDIO, static_cast< unsigned int >( i ), level[ i ] );
...
int runPipe( int width, int height, int fps, const std::string& scriptPath, const Options& options )
{
	...
	for( const auto& entry : tracks )
	{
		...
		if( found == byName.end() )
		{
			std::fprintf( stderr, "frtest: no parameter named \"%s\" in the script\n",
			              entry.first.c_str() );
			return 1;
		}
	...
	while( readExactly( incoming.data(), frameBytes ) )
	{
		...
		{
			std::fprintf( stderr, "frtest: render failed at frame %d\n", frame );
			return 1;
		}
		...
		{
			std::fprintf( stderr, "frtest: short write at frame %d\n", frame );
			return 1;
		}
	...
	std::fflush( stdout );
	std::fprintf( stderr, "frtest: %d frames\n", frame );
...
int main( int argc, char** argv )
{
	...
	for( int i = 1; i < argc; ++i )
	{
		...
		if( arg == "--out" )
			...
			fps = std::max( 1, std::atoi( next().c_str() ) );
		else if( arg == "--size" )
		{
	...
	bool needsGL = wantList || wantPipe || !outPath.empty() || !scenePath.empty() || checks.empty();
	...
	if( wantPipe )
	{
		const int code = runPipe( options.width, options.height, fps, scriptPath, options );
