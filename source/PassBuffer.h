...
/**
    ...
    **It reallocates only when it has to.** `Ensure()` is called every frame and
    is a no-op in the overwhelming majority of them. Ferric's buffers are sized
    ...
    **It actually frees its colour texture.** `ffglex::FFGLFBO::Release()`
    ...
    Ferric is safe by construction -- `ProcessOpenGL` calls every `Ensure()`
    ...
    There is no mipmap helper here, unlike the fleet's blur-shaped plugins.
    Nothing in Ferric samples a reduced level: the compander's high-pass is a
    fixed set of taps along one axis, and the warp reads at most one texel away
    from where it was told. A mip chain would be built every frame and read by
    nobody.
*/
